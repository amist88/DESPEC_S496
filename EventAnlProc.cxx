...
// Adapted for DESPEC by A.K.Mistry 2021
//-----------------------------------------------------------------------
...
//            }
        ...
        pOutput->pbeta_vftx_2141[i] = pInput->fbeta_vftx_2141[i];
        pOutput->pgamma_vftx_2141[i] = pInput->fgamma_vftx_2141[i];
        pOutput->paoq_vftx_2141[i] = pInput->faoq_vftx_2141[i];
        pOutput->paoq_corr_vftx_2141[i] = pInput->faoq_corr_vftx_2141[i];
        pOutput->pz1_vftx_2141[i] = pInput->fz1_vftx_2141[i];
        pOutput->pz2_vftx_2141[i] = pInput->fz2_vftx_2141[i];
        pOutput->pvcor_vftx_2141[i] = pInput->fvcor_vftx_2141[i];
        pOutput->pbeta_vftx_2241[i] =  pInput->fbeta_vftx_2241[i];
        pOutput->pgamma_vftx_2241[i] = pInput->fgamma_vftx_2241[i]; 
        pOutput->paoq_vftx_2241[i] =  pInput->faoq_vftx_2241[i];
        
        pOutput->paoq_corr_vftx_2241[i] = pInput->faoq_corr_vftx_2241[i];
        pOutput->pz1_vftx_2241[i] = pInput->fz1_vftx_2241[i];
        pOutput->pz2_vftx_2241[i] = pInput->fz2_vftx_2241[i];
        pOutput->pvcor_vftx_2241[i] = pInput->fvcor_vftx_2241[i];
      }
        for(int i = 0; i < 7; i++)
        {
...
//         FRS_beta3 = pInput->fFRS_beta3;
      //  FRS_gamma  = pInput->fFRS_gamma;
        ...
        else if (MHTDC_TAC_VFTX==2){
        
            ///I take 22->41 for now
            for(int i=0; i<32;i++){
              // if(pInput->faoq_vftx_2241[i]>0) cout<<"1 pInput->faoq_vftx_2241[i] " <<pInput->faoq_vftx_2241[i] << " i " << i <<endl;
            if(pInput->faoq_vftx_2241[i] > 1 && pInput->faoq_vftx_2241[i]<3.5){
              // if(pInput->faoq_vftx_2241[i]>0)  cout<<"2 pInput->faoq_vftx_2241[i] " <<pInput->faoq_vftx_2241[i] << " i " << i <<endl;
        FRS_AoQ = pInput->faoq_vftx_2241[i];
        FRS_AoQ_corr = pInput->faoq_corr_vftx_2241[i];
        FRS_z = pInput->fz1_vftx_2241[i];
        FRS_z2 = pInput->fz2_vftx_2241[i];
        ///Not gotten around to implementing this for vftx yet...
        FRS_dEdeg = pInput->fFRS_dEdeg;
        FRS_dEdegoQ = pInput->fFRS_dEdegoQ;
        FRS_beta = pInput->fbeta_vftx_2241[i];
                }
            }
        }
        else {cout<<"TOF/TAC/VFTX Parameter not set correctly in DESPEC_Setup_File.h"<<endl; exit(0);}
...
//                     bPlas_RefCh0_Det3[k] = pInput->fbPlas_Lead_PMT[3][bPlastRefCh_Det3][k];
  ...
  Do_Plastic_Twinpeaks_Histos(pInput,pOutput);
...
for (int i=0; i<FAT_VME_MAX_MULTI; i++){
  ...
  if ( PrcID_Conv[3]==3 ){
    ...
    SC41L_mult =  pInput->fFat_SC41L_mult;
    SC41R_mult =  pInput->fFat_SC41R_mult; 
    SC41L_dig_mult =  pInput->fFat_SC41L_dig_mult;
    SC41R_dig_mult =  pInput->fFat_SC41R_dig_mult; 
...
//   }
//
        for(int i = 0; i < SC41L_mult; i++){
        SC41L[i] = pInput->fSC41L[i]; 
        }//End sc41 for fill
        
        for(int i = 0; i < SC41R_mult; i++){
            SC41R[i] = pInput->fSC41R[i]; 
        }//End sc41 for fill

 
        for(int i = 0; i < SC41L_dig_mult; i++){
        SC41L_dig[i] = pInput->fSC41L_dig[i]; 
        }//End sc41 for fill
        
        for(int i = 0; i < SC41R_dig_mult; i++){
            SC41R_dig[i] = pInput->fSC41R_dig[i]; 
        }//End sc41 for fill
...
//     MakeTH2('D', "bPlastic/FIMP_ToT_Correlation_Comb1", "ToT vs ToT for 2 FIMP channels, combination 1",500,0,100000,500,0,100000);
    ...
    hID_Z2_AoQ = MakeTH2('D',"FRS/ID/ID_Z2_AoQ", "Z2 vs A/Q",1500,frs_id->min_aoq_plot,frs_id->max_aoq_plot, 1000,frs_id->min_z_plot,frs_id->max_z_plot,"A/Q s2-s4", "Z2 s2-s4");
    
    hID_Z1_vs_T = MakeTH2('D',"FRS/ID/ID_Z1_Time", "Z1 vs Time",1705,11950,29000, 1500,frs_id->min_z_plot,frs_id->max_z_plot,"Time (/10 mins)", "Z1 s2-s4");
    ...
    //MARTA
    hID_Z_E_SC42 = MakeTH2('D',"FRS/ID/ID_Z1_E_SC42","Z1 vs. E SC42", 2000,frs_id->min_z_plot,frs_id->max_z_plot, 400,0,4000,"Z1", "E_SC42");
    
    hID_Z_E_SC43 = MakeTH2('D',"FRS/ID/ID_Z1_E_SC43","Z1 vs. E SC43", 2000,frs_id->min_z_plot,frs_id->max_z_plot, 400,0,4000,"Z1", "E_SC43");
...
void EventAnlProc::Do_FRS_Histos(EventUnpackStore* pInput, EventAnlStore* pOutput){
    FRS_time_mins = 0;
    
    if(pOutput->pFRS_WR>0) FRS_time_mins =(pOutput->pFRS_WR/60E9)-27000000;
   ...
   if(FRS_AoQ>0 && FRS_z2>0) hID_Z2_AoQ->Fill(FRS_AoQ, FRS_z2);
   ...
   if(FRS_z>0 && FRS_sci_e[6]) hID_Z_E_SC42->Fill(FRS_z,FRS_sci_e[6]);

   if(FRS_z>0 && FRS_sci_e[7]) hID_Z_E_SC43->Fill(FRS_z,FRS_sci_e[7]);
   ...
   if(FRS_z>0 && FRS_dEdeg!=0)  hdEdeg_Z->Fill(FRS_z, FRS_dEdeg);
     ...
     for(int i=0; i<MAX_FRS_GATE; i++){
      ...
      if(cID_x4AoQ[i]->Test(FRS_AoQ, FRS_ID_x4)==true)
       ...
       if( cID_Z_AoQ[g]->Test(FRS_AoQ, FRS_z2)==true){
       ...
       // cout<<"event_number " << pOutput->pEvent_Number<<" pOutput->pFRS_ZAoQ_pass[g] " <<  pOutput->pFRS_ZAoQ_pass[g] << " g " <<g<<endl;
         hID_Z_AoQgate[g]->Fill(FRS_AoQ, FRS_z2);
...
//    cout<<"pInput->fbPlas_Lead_PMT[bPLASTIC_ADDITIONAL_CH_MOD][bPLASTIC_UP_COIN][j] "<< pInput->fbPlas_Lead_PMT[bPLASTIC_ADDITIONAL_CH_MOD][bPLASTIC_UP_COIN][j] << endl;
     ...
     if(a==bPLASTIC_UPSTREAM_DET && pInput->fbPlas_Lead_PMT[bPLASTIC_ADDITIONAL_CH_MOD][bPLASTIC_UP_COIN][0] != 0&&  lead_bplas[bPLASTIC_UPSTREAM_DET][b][j]!=0){ // s452 a==2 is the upstream detector. Coincidence is in a=3, b=1.
        ...
        hbPlas_Lead_dT_coinc[bPLASTIC_UPSTREAM_DET][b]->Fill((lead_bplas[bPLASTIC_UPSTREAM_DET][b][j] - pInput->fbPlas_Lead_PMT[bPLASTIC_ADDITIONAL_CH_MOD][bPLASTIC_UP_COIN][0])*5);
          cout<<"dT " <<(lead_bplas[bPLASTIC_UPSTREAM_DET][b][j] - pInput->fbPlas_Lead_PMT[bPLASTIC_ADDITIONAL_CH_MOD][bPLASTIC_UP_COIN][0])*5 << " lead_bplas[bPLASTIC_UPSTREAM_DET][b][j] " <<lead_bplas[bPLASTIC_UPSTREAM_DET][b][j] << " pInput->fbPlas_Lead_PMT[bPLASTIC_ADDITIONAL_CH_MOD][bPLASTIC_UP_COIN][0] " <<pInput->fbPlas_Lead_PMT[bPLASTIC_ADDITIONAL_CH_MOD][bPLASTIC_UP_COIN][0] << endl;
...
//                     if(i>15 && pInput->fbPlas_Lead_PMT[16][j]>0 && pInput->fbPlas_Lead_PMT[a][b][j]>0) {   
      ...
      ///**---------------------------------------------Plastic Trail ----------------------------------**/  
               ...
               // if(j<20){
                  ...
                  if(pInput->fbPlas_Trail_PMT[a][b][j]!=0)  hits_bplas_trail++;  
   ...
   ///**--------------------Plastic ToT --------------------------------**/  
    ...
    /////////////////////////////////////////////////// 
    void EventAnlProc::Do_Plastic_Twinpeaks_Histos(EventUnpackStore* pInput, EventAnlStore* pOutput){   
 ...
 void EventAnlProc::Make_Fatima_Tamex_Histos(){
     ...
     for(int i=1; i<=FATIMA_TAMEX_CHANNELS; i++){ 
        ...
        // hFat_ToT_Fast_det[i]=   MakeTH1('D', Form("FATIMA_TAMEX/Fast/ToT/ToT Fast Ch.%2d",i), Form("ToT Fast %2d",i), 5000,0,150000);      
         ...
         hFat_ToT_Slow_det[i]=   MakeTH1('D', Form("FATIMA_TAMEX/Slow/ToT/ToT Slow Ch.%2d",i), Form("ToT Slow %2d",i), 2000,0,20000);     
...
void EventAnlProc::Make_Fatima_Histos(){
  ...
  for (int i=0; i<FAT_MAX_VME_CHANNELS; i++){
    ...
    hFat_SC41L_TDC_dT[i] = MakeTH1('D', Form("FATIMA_VME/Timing/SC41-TDC_dT/TDCdT_SCI41_Cha_LaBr%02d", i), Form("TDC dt Channel All Multip SCI41 - LaBr%02d", i),1e4,-2E5,2E5);
  // for (int j=0; j<6; j++){
  ...
  // }
    ...
    hFAT_hits_QDC       = MakeTH1('D', "FATIMA_VME/Stats/QDC_FAThits", "FATIMA QDC hit pattern",54,0,54);
    hFAT_hits_TDC       = MakeTH1('D', "FATIMA_VME/Stats/TDC_FAThits", "FATIMA TDC hit pattern",54,0,54);
    hFAT_Multipl       = MakeTH1('D', "FATIMA_VME/Stats/Fatima_VME_Multiplicity", "FATIMA TDC Multiplicity",50,0,50);

     hFat_Energy_GainMonitor= MakeTH2('D',"FATIMA_VME/Stats/Fatima_GainMonitor","FATIMA Energy-Time Matrix",1705,11950,29000,2500,0,5000,"Time (minutes)","Fat Energy (keV)");


   // hFAT_SC41= MakeTH2('D',"FATIMA_VME/SC41/SC41","SC41 L and R",1e3,-10,90,1e3,-10,90,"us","us");
    hFAT_SC41L= MakeTH2('D',"FATIMA_VME/SC41/SC41L","SC41L L and R",4e3,-10000,90000,4e3,-10000,90000,"L ns","R ns");
    hFAT_SC41R= MakeTH2('D',"FATIMA_VME/SC41/SC41R","SC41R L and R",4e3,-10000,90000,4e3,-10000,90000,"L ns","R ns");
    hFAT_SC41_LR_all= MakeTH2('D',"FATIMA_VME/SC41/SC41L_and_R","SC41 LR L and R",4e3,-10000,90000,4e3,-10000,90000,"L ns","R ns");

    hFAT_SC41L_digi= MakeTH2('D',"FATIMA_VME/SC41/SC41L_digi","SC41L digi L and R",4e3,-10000,90000,4e3,-10000,90000,"L ns","R ns");
    hFAT_SC41R_digi= MakeTH2('D',"FATIMA_VME/SC41/SC41R_digi","SC41R digi L and R",4e3,-10000,90000,4e3,-10000,90000,"L ns","R ns");
    hFAT_SC41_LR_all_digi= MakeTH2('D',"FATIMA_VME/SC41/SC41L_and_R_digi","SC41 LR digi L and R",4e3,-10000,90000,4e3,-10000,90000,"L ns","R ns");

    hFAT_SC41R_digi_gamma= MakeTH2('D',"FATIMA_VME/SC41/SC41L_digi_gamma","SC41R digi R and gamma in Labr 0",4e3,-10000,90000,4e3,-10000,90000,"Gamma in LaBr ns","SC41R ns");
    hFAT_SC41R_dT_energy_gamma= MakeTH2('D',"FATIMA_VME/SC41/SC41L_dT_gamma","SC41R dT and gamma in Labr 0",4e3,-10000,90000,600,0,3000,"dT Gamma in LaBr with SC41R ns","Gamma Energy keV");
    hFAT_gamma_energy_time= MakeTH2('D',"FATIMA_VME/SC41/gamma_energy_gated_time_Labr0","gamma energy vs time Labr0",4e3,-10000,60000,600,0,3000,"Gamma time ns","Gamma Energy keV");

    hFAT_SC41L_mult= MakeTH1('D',"FATIMA_VME/SC41/SC41L_multi","SC41L multi",10,0,10);
    hFAT_SC41R_mult= MakeTH1('D',"FATIMA_VME/SC41/SC41R_multi","SC41R multi",10,0,10);
    hFAT_SC41L_digi_mult= MakeTH1('D',"FATIMA_VME/SC41/SC41L_digi_multi","SC41L multi",10,0,10);
    hFAT_SC41R_digi_mult= MakeTH1('D',"FATIMA_VME/SC41/SC41R_digi_multi","SC41R multi",10,0,10);
...
void EventAnlProc::Do_Fatima_Histos(EventUnpackStore* pInput, EventAnlStore* pOutput){
    ...
    Fat_time_mins =0; 
    if(Fat_WR>0) Fat_time_mins =(Fat_WR/60E9)-27000000;
   ...
   // double Fat_bPlastCh_dT[FAT_MAX_VME_CHANNELS][6];
    ...
    hFAT_SC41L_mult->Fill(SC41L_mult); 
    hFAT_SC41R_mult->Fill(SC41R_mult); 

    for(int i = 0; i < SC41L_mult; i++){
    if(SC41R[i]>0 && SC41L[i]>0) hFAT_SC41L->Fill(SC41L[i]*25/1.0e3,SC41R[i]*25/1.0e3); 
    }
    for(int i = 0; i < SC41R_mult; i++){
    if(SC41L[i]>0 && SC41R[i]>0) hFAT_SC41R->Fill(SC41L[i]*25/1.0e3,SC41R[i]*25/1.0e3); 
    }
    for(int i = 0; i < SC41L_mult; i++){
     for(int j = 0; j < SC41R_mult; j++){
     if(SC41L[i]>0&&SC41R[j]>0) hFAT_SC41_LR_all->Fill(SC41L[i]*25/1.0e3,SC41R[j]*25/1.0e3); 
     }
    }

    hFAT_SC41L_digi_mult->Fill(SC41L_dig_mult); 
    hFAT_SC41R_digi_mult->Fill(SC41R_dig_mult); 

    for(int i = 0; i < SC41L_dig_mult; i++){
    if(SC41R_dig[i]>0 && SC41L_dig[i]>0) hFAT_SC41L_digi->Fill(SC41L_dig[i]*25/1.0e3,SC41R_dig[i]*25/1.0e3); 
    }
    for(int i = 0; i < SC41R_dig_mult; i++){
    if(SC41L_dig[i]>0 && SC41R_dig[i]>0) hFAT_SC41R_digi->Fill(SC41L_dig[i]*25/1.0e3,SC41R_dig[i]*25/1.0e3); 
    }
    for(int i = 0; i < SC41L_dig_mult; i++){
     for(int j = 0; j < SC41R_dig_mult; j++){
     if(SC41L_dig[i]>0&&SC41R_dig[j]>0) hFAT_SC41_LR_all_digi->Fill(SC41L_dig[i]*25/1.0e3,SC41R_dig[j]*25/1.0e3); 
     }
    }
    ...
    if(Fat_time_mins>0 && Fat_QDC_E[i]>0) hFat_Energy_GainMonitor->Fill(Fat_time_mins,Fat_QDC_E[i]);
          ...
          /// Fatima Time PMT - PMT Ch.x Energy gated
                for(int j = 0; j < SC41L_mult; j++){
                   Fat_Ch_SC41L_dT[Fat_TDC_ID[i]] =  (SC41L[j] - Fat_TDC_T[i])*25;
                   if( Fat_Ch_SC41L_dT[Fat_TDC_ID[i]]!=0) 
                   {hFat_SC41L_TDC_dT[Fat_TDC_ID[i]]->Fill(Fat_Ch_SC41L_dT[Fat_TDC_ID[i]] );// 1ps
                   if(Fat_TDC_ID[i]==0&&Fat_TDC_T[i]>0&&Fat_QDC_E[i]>0){
                      hFAT_SC41R_digi_gamma->Fill(Fat_TDC_T[i]*25/1.0e3,SC41R[j]*25/1.0e3);//ns range
                      hFAT_SC41R_dT_energy_gamma->Fill(Fat_Ch_SC41L_dT[Fat_TDC_ID[i]]*25/1.0e3,Fat_QDC_E[i]);//ns range
                      }
                   }
            }
                   ...
                   if(Fat_TDC_ID[i]==0&&Fat_TDC_T[i]>0&&Fat_QDC_E[i]>0){
                      hFAT_gamma_energy_time->Fill(Fat_TDC_T[i]*25/1.0e3,Fat_QDC_E[i]);//ns range
                      }

      double t2, t1;
     // if(Fat_TDC_T[i]>0&&Fat_QDC_E[i]>30){
      if(Fat_TDC_T[i]>0){
    t1 = Fat_TDC_T[i];
  
    if (Fat_TDC_ID[i] == 1){
           for(int j = 0; j < Fatmult; j++){
                  // if(Fat_TDC_T[j]>0&&Fat_QDC_E[j]>30){
                   if(Fat_TDC_T[j]>0){
      ...
      pOutput->pSC41L_dig_mult =  SC41L_dig_mult;
      pOutput->pSC41R_dig_mult =  SC41R_dig_mult; 
      pOutput->pSC41L_mult =  SC41L_mult;
      pOutput->pSC41R_mult =  SC41R_mult; 

 
        for(int i = 0; i < SC41L_mult; i++){
            pOutput->pSC41L[i] = SC41L[i];    
        }//End sc40 for fill
        
        for(int i = 0; i < SC41R_mult; i++){
            pOutput->pSC41R[i] = SC41R[i];    
        }//End sc41 for fill        

        for(int i = 0; i < SC41L_dig_mult; i++){
            pOutput->pSC41L_dig[i] = SC41L_dig[i];    
        }//End sc40 for fill
        
        for(int i = 0; i < SC41R_dig_mult; i++){
            pOutput->pSC41R_dig[i] = SC41R_dig[i];    
        }//End sc41 for fill        
  ...
  //  cout<<"pInput->fFat_TMCh1mult " << pInput->fFat_TMCh1mult <<endl;
    ...
    void EventAnlProc::Make_Germanium_Histos()
    {
      ...
      hGe_SC41L_Mult = MakeTH1('I',"Germanium/SCI41/Germanium_SC41L_Ana_Mult","Germanium SC41 Ana Left Mult",10,0,10);
      hGe_SC41R_Mult = MakeTH1('I',"Germanium/SCI41/Germanium_SC41R_Ana_Mult","Germanium SC41 Ana Right Mult",10,0,10);
      hGe_SC41L_digi_Mult = MakeTH1('I',"Germanium/SCI41/Germanium_SC41L_Digi_Mult","Germanium SC41 Digi Left Mult",10,0,10);
      hGe_SC41R_digi_Mult = MakeTH1('I',"Germanium/SCI41/Germanium_SC41R_Digi_Mult","Germanium SC41 Digi Right Mult",10,0,10);

//     hGe_SC41L = MakeTH1('I',"Germanium/SCI41/Germanium_SC41L_Ana","Germanium SC41 Left",400,0,400);
//     hGe_SC41R = MakeTH1('I',"Germanium/SCI41/Germanium_SC41R_Ana","Germanium SC41 Right",400,0,400);
//     hGe_SC41L_digi = MakeTH1('I',"Germanium/SCI41/Germanium_SC41L_Digi","Germanium SC41 Left",400,0,400);
//     hGe_SC41R_digi = MakeTH1('I',"Germanium/SCI41/Germanium_SC41R_Digi","Germanium SC41 Right",400,0,400);

//     hGe_SC41 = MakeTH2('D',"Germanium/SCI41/Germanium_SC41_EE_Ana","Germanium SC41 Right vs Left",400,0,400,400,0,400,"Right","Left");
//     hGe_SC41_digi = MakeTH2('D',"Germanium/SCI41/Germanium_SC41_EE_Digi","Germanium SC41 digi Right vs Left",400,0,400,400,0,400,"Right","Left");

      hGe_SC41_time = MakeTH2('D',"Germanium/SCI41/Germanium_SC41_TT_Ana_time","Germanium SC41 time Right vs Left",2000,0,2000,2000,0,2000,"Right","Left");
      hGe_SC41_digi_time = MakeTH2('D',"Germanium/SCI41/Germanium_SC41_TT_Digi_time","Germanium SC41 digi time Right vs Left",2000,0,2000,2000,0,2000,"Right","Left");
      hGe_SC41_timediff = MakeTH1('I',"Germanium/SCI41/Germanium_SC41_time_diff","Germanium SC41 time diff",400,-200,200);
      hGe_SC41_digi_timediff = MakeTH1('I',"Germanium/SCI41/Germanium_SC41_Digi_time_diff","Germanium SC41 Digi time diff",400,-200,200);
      ...
      hGe_EE_Mat_1 = MakeTH2('D',"Germanium/matrix/Germanium_EE_Mat_1","Germanium Energy-Energy Matrix crystal before AddBack",2000,0,2000,2000,0,2000);
      hGe_EE_Mat_2 = MakeTH2('D',"Germanium/matrix/Germanium_EE_Mat_2","Germanium Energy-Energy Matrix crystal after AddBack",2000,0,2000,2000,0,2000);
      ...
      hGe_Energy_GainMonitor= MakeTH2('D',"Germanium/Stats/Germanium_GainMonitor","Germanium Energy-Time Matrix",1705,11950,29000,2500,0,5000,"Time (minutes)","Ge Energy (keV)");
//      hGe_MultvsdT = MakeTH2('D',"Germanium/Stats/Germanium_Mult_vs_GamGamdT","Germanium Multiplicity vs Gamma Gamma dT",Germanium_MAX_HITS,0,Germanium_MAX_HITS,200,-1000,1000);
      ...
      hGe_dTvsChan = MakeTH2('D',"Germanium/Time_diff/Germanium_dT_vs_Chan","Germanium dT vs Chan",30,0,30,200,-1000,1000);
    
      for (int i=0; i<Germanium_MAX_DETS; i++)
      {
        for (int j = 0; j < Germanium_CRYSTALS; j++)
        {
//	  hGe_nocalib[i][j] = MakeTH1('D',Form("Germanium/Ge_nocalib/Germanium_nocalib_Det_%2d_%1d",i, j), Form("Germanium No Calibration Detector %2d Crystal %1d",i, j),20000,0,2000000);
	  
          hGe_Chan_E[i][j] = MakeTH1('D',Form("Germanium/Energy_Ch_1keV/Germanium_E_Det_%2d_%1d",i, j), Form("Germanium Channel Energy Detector %2d Crystal %1d",i, j),5000,0,5000);//20000,0,2000000);
          hGe_Chan_E_Addback[i][j] = MakeTH1('D',Form("Germanium/Energy_Ch_1keV/Germanium_E_Addback_Det_%2d_%1d",i, j), Form("Germanium Channel Energy Addback Detector %2d Crystal %1d",i, j),5000,0,5000);//20000,0,2000000);
   ...
   hGe_Chan_Time[i][j] = MakeTH1('D',Form("Germanium/Time/Germanium_Chan_Time_Det_%2d_Chan_%2d",i,j), Form("Germanium Channel Time for Detector %2d Channel %2d",i,j),1000,0,10000);
   hGe_Chan_CF_Time[i][j] = MakeTH1('D',Form("Germanium/Time/Germanium_Chan_CF_Time_Det_%2d_Chan_%2d",i,j), Form("Germanium Channel CF Time for Detector %2d Channel %2d",i,j),1000,0,10000);
   ...
   hGe_dTvsE[i][j] = MakeTH2('D',Form("Germanium/Time_diff/Germanium_dT_vs_Energy_Det_%2d_Crys_%2d",i,j),Form("Germanium dT vs Gamma Energy det %2d crys %2d",i,j),1000,0,2000,200,-1000,1000);
          
          hGe_Chan_mult[i][j] = MakeTH1('D',Form("Germanium/mult/Germanium_mult_Det_%2d_%1d",i, j), Form("Germanium Channel mult Detector %2d Crystal %1d",i, j),10,0,10);//20000,0,2000000);
  // hGe_Chan_Time_Diff_CF[i][j] = MakeTH1('D',Form("Germanium/Time_diff_CF/Germanium_Chan_Time_CF_Diff_Det_%2d_Chan_%2d",i,j), Form("Germanium Channel Time Difference with Const Frac for Detector %2d Channel %2d",i,j),2000,-1000,1000);
    ...
    {
        Ge_time_mins=0;
      if(Ge_WR>0) Ge_time_mins =(Ge_WR/60E9)-27000000;
...
//         timeFIRST_Ge=0;
       ...
       //check multi of SC41 L and R for ana and digi;
        mult_sc41L_ana=0;
        mult_sc41R_ana=0;
        mult_sc41L_digi=0;
        mult_sc41R_digi=0;
       ...
       long tmp_dt;
        for (int i = 0; i < GeFired; i++){
              ...
              int det = GeDet[i];
              int crys = GeCrys[i];
//          if( Ge_cfd_Talign[i]>0){
//          cout<<"CFD time vs timestamp"<<endl;
//           cout<<Ge_cfd_Talign[i]<<endl;
//           cout<<Ge_Talign[i]<<endl;
//           cin.get();
       //SC41 signal
          // if(det==Germanium_SC41_Det  && crys==Germanium_SC41L_Crystal && Ge_Talign[i]>0 && GeE_Cal[i]>0) {mult_sc41L_ana++; Ge_SC41_ana_T[0][mult_sc41L_ana]=Ge_Talign[i]; }
           if(det==Germanium_SC41_Det  && crys==Germanium_SC41L_Crystal && Ge_Talign[i]>0 ) { Ge_SC41_ana_T[0][mult_sc41L_ana]=Ge_Talign[i]; mult_sc41L_ana++;}
           if(det==Germanium_SC41_Det  && crys==Germanium_SC41R_Crystal && Ge_Talign[i]>0 ) { Ge_SC41_ana_T[1][mult_sc41R_ana]=Ge_Talign[i]; mult_sc41R_ana++;}
           if(det==Germanium_SC41_Det_Digi && crys==Germanium_SC41L_Crystal_Digi && Ge_Talign[i]>0 ) { Ge_SC41_dig_T[0][mult_sc41L_digi]=Ge_Talign[i]; mult_sc41L_digi++;} 
           if(det==Germanium_SC41_Det_Digi && crys==Germanium_SC41R_Crystal_Digi && Ge_Talign[i]>0 ) { Ge_SC41_dig_T[1][mult_sc41R_digi]=Ge_Talign[i]; mult_sc41R_digi++;} 
           if(det>-1 && det<4 && GeE_Cal[i]>30 && Ge_Talign[i]>0) mult_Ge[det][crys]++;
	   ...
	   if(det>-1){
          ...
          // hGe_Chan_Time[det][crys]->Fill(Ge_Talign[i]/1.0e9-273000);// in the second unit for calibration run before exp
           hGe_Chan_Time[det][crys]->Fill(Ge_Talign[i]/1.0e9-374000);// in the second unit for run 60
           hGe_Chan_CF_Time[det][crys]->Fill(Ge_cfd_Talign[i]/1.0e9);// in the second unit for run 60
      //    cout<<(Ge_Talign[i]/1.0e9-374000)<<endl;
      //    cout<<(Ge_cfd_Talign[i]/1.0e9)<<endl;
      //    cin.get(); 
           }
           // Cross correlations
    if ( det!=Germanium_TimeMachine_Det&& det!=Germanium_SC41_Det  && det!=Germanium_SC41_Det_Digi){
        Gam_mult++;
           ...
           {
             if(i == j) continue;
              if(GeDet[j]==0&&GeCrys[j]==0&&GeE_Cal[i]>30 && GeE_Cal[j]>30 && Ge_Talign[i]>0 && Ge_Talign[j]>0){
                 dT_Align= Ge_Talign[i]-Ge_Talign[j];
                  hGe_Chan_Time_Diff[det][crys]->Fill(dT_Align);
                  tmp_id=det*7+crys;
                  if(tmp_id<30)  hGe_dTvsChan->Fill(tmp_id,dT_Align);
                  hGe_dTvsE[det][crys]->Fill(GeE_Cal[i],dT_Align);
    ...
    }//end the loop for Gefired
       hGe_Mult->Fill(Gam_mult);
       //hGe_MultvsdT->Fill(Gam_mult,dT_CFD_Align);
       //
       for (int i = 0; i < Germanium_MAX_DETS; i++){
           for (int j = 0; j < Germanium_CRYSTALS; j++){
        hGe_Chan_mult[i][j]->Fill(mult_Ge[i][j]);
        }
       }

       //SC41 signal
       hGe_SC41L_Mult->Fill(mult_sc41L_ana);          
       hGe_SC41R_Mult->Fill(mult_sc41R_ana);
       hGe_SC41L_digi_Mult->Fill(mult_sc41L_digi);
       hGe_SC41R_digi_Mult->Fill(mult_sc41R_digi);

       //analysis of multi-hit SC41 in Ge

       pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41L_Crystal]=0;
       pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41R_Crystal]=0;
       pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41L_Crystal_Digi]=0;
       pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41R_Crystal_Digi]=0;

       for(int i=0; i<mult_sc41L_ana; i++){
      // for(int i=0; i<5; i++){
       tmp_dt=Ge_SC41_ana_T[0][i]-Ge_SC41_ana_T[1][i];
//      cout<<"multi sc41 "<<i<<endl;
//      cout<<Ge_SC41_ana_T[0][i]<<endl;
//      cout<<Ge_SC41_ana_T[1][i]<<endl;
//      cout<<Ge_SC41_dig_T[0][i]<<endl;
//      cout<<Ge_SC41_dig_T[1][i]<<endl;
//      cout<<"Time diff == "<<tmp_dt<<endl;
//      cin.get();
        if(abs(tmp_dt)<15&&Ge_SC41_ana_T[0][i]>0&&Ge_SC41_ana_T[1][i]>0){
        pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41L_Crystal]=Ge_SC41_ana_T[0][i];
        pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41R_Crystal]=Ge_SC41_ana_T[1][i];
        }
       tmp_dt=Ge_SC41_dig_T[0][i]-Ge_SC41_dig_T[1][i];
        if(abs(tmp_dt)<15&&Ge_SC41_dig_T[0][i]>0&&Ge_SC41_dig_T[1][i]>0){
        pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41L_Crystal_Digi]=Ge_SC41_dig_T[0][i];
        pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41R_Crystal_Digi]=Ge_SC41_dig_T[1][i];
         }
       }
 //      cout<<"####event stop####"<<endl;

    //      if(pOutput->pGe_E[Germanium_SC41_Det][Germanium_SC41L_Crystal]>0) hGe_SC41L->Fill(pOutput->pGe_E[Germanium_SC41_Det][Germanium_SC41L_Crystal]);
    //      if(pOutput->pGe_E[Germanium_SC41_Det][Germanium_SC41R_Crystal]>0) hGe_SC41R->Fill(pOutput->pGe_E[Germanium_SC41_Det][Germanium_SC41R_Crystal]);
    //      if(pOutput->pGe_E[Germanium_SC41_Det_Digi][Germanium_SC41R_Crystal_Digi]>0) hGe_SC41R_digi->Fill(pOutput->pGe_E[Germanium_SC41_Det_Digi][Germanium_SC41R_Crystal_Digi]);
    //      if(pOutput->pGe_E[Germanium_SC41_Det_Digi][Germanium_SC41L_Crystal_Digi]>0) hGe_SC41L_digi->Fill(pOutput->pGe_E[Germanium_SC41_Det_Digi][Germanium_SC41L_Crystal_Digi]);
               
       // if(pOutput->pGe_E[Germanium_SC41_Det][Germanium_SC41R_Crystal]>0&&pOutput->pGe_E[Germanium_SC41_Det][Germanium_SC41L_Crystal]>0&&pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41R_Crystal]>0&&pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41L_Crystal]>0){
        if(pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41R_Crystal]>0&&pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41L_Crystal]>0){
           //   hGe_SC41->Fill(pOutput->pGe_E[Germanium_SC41_Det][Germanium_SC41L_Crystal],pOutput->pGe_E[Germanium_SC41_Det][Germanium_SC41R_Crystal]);
              hGe_SC41_time->Fill((pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41R_Crystal])/1.0e9-374000,(pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41L_Crystal])/1.0e9-374000);
              tmp_dt=pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41R_Crystal]-pOutput->pGe_T[Germanium_SC41_Det][Germanium_SC41L_Crystal];
              hGe_SC41_timediff->Fill(tmp_dt);
               }
             // if(pOutput->pGe_E[Germanium_SC41_Det_Digi][Germanium_SC41R_Crystal_Digi]>0&&pOutput->pGe_E[Germanium_SC41_Det_Digi][Germanium_SC41L_Crystal_Digi]>0&&pOutput->pGe_T[Germanium_SC41_Det_Digi][Germanium_SC41R_Crystal_Digi]>0&&pOutput->pGe_T[Germanium_SC41_Det_Digi][Germanium_SC41L_Crystal_Digi]>0){
              if(pOutput->pGe_T[Germanium_SC41_Det_Digi][Germanium_SC41R_Crystal_Digi]>0&&pOutput->pGe_T[Germanium_SC41_Det_Digi][Germanium_SC41L_Crystal_Digi]>0){
             // hGe_SC41_digi->Fill(pOutput->pGe_E[Germanium_SC41_Det_Digi][Germanium_SC41L_Crystal_Digi],pOutput->pGe_E[Germanium_SC41_Det_Digi][Germanium_SC41R_Crystal_Digi]);
              hGe_SC41_digi_time->Fill((pOutput->pGe_T[Germanium_SC41_Det_Digi][Germanium_SC41R_Crystal_Digi])/1.0e9-374000,(pOutput->pGe_T[Germanium_SC41_Det_Digi][Germanium_SC41L_Crystal_Digi])/1.0e9-374000);
              tmp_dt=pOutput->pGe_T[Germanium_SC41_Det_Digi][Germanium_SC41R_Crystal_Digi]-pOutput->pGe_T[Germanium_SC41_Det_Digi][Germanium_SC41L_Crystal_Digi];
              hGe_SC41_digi_timediff->Fill(tmp_dt);
               }
      

               // gamma - gamma matrix before addback
        for (int i = 0; i < GeFired; i++){
           for (int j = 0; j < GeFired; j++){
            if(i==j) continue;
            //if(GeDet[i]==0 && GeDet[j]==0 && GeCrys[i]==0 && GeCrys[j]==2){
                 if(abs(Ge_Talign[i]-Ge_Talign[j])<200 && GeE_Cal[i]>30 && GeE_Cal[j]>30 && GeE_Cal[i] < 2000 && GeE_Cal[j] < 2000 ){
            hGe_EE_Mat_1->Fill(GeE_Cal[i],GeE_Cal[j]);
             //      }
                 }
           }
        }
       ...
       ////ADDBACK
        static const long dT_addback = 400;
        static const double energy_thres_addback = 30;
               
        // Detector addback
            double adbk_E ;
            long adbk_T ;
         //   long T[Germanium_CRYSTALS] ;
            int crys[Germanium_CRYSTALS] ;
            int v ;

              //  cout<<"Addback started"<<endl;
                for (int i = 0; i < GeFired; i++){
                 if(GeDet[i]>-1 && GeDet[i]<4 && Ge_Talign[i]>0 && GeE_Cal[i]>energy_thres_addback){
                 v=0;
              //   T[Germanium_CRYSTALS] = { 0 };
                 crys[Germanium_CRYSTALS] = { 0 };
          ...
          //          T[v] = Ge_Talign[i];
                ...
                //    n[v] = 1;
                    v++;
                 for (int j = i+1; j < GeFired; j++){
                   if( Ge_Talign[j]>0 && GeE_Cal[j]>energy_thres_addback && GeDet[i]==GeDet[j] && v<5){
                tmp_dt=Ge_Talign[i] - Ge_Talign[j];
               if (abs(tmp_dt) < dT_addback){
                int tmp_addback_hit=v;
               for(int k = 0; k<tmp_addback_hit ; k++){
                 if( abs(crys[k]-GeCrys[j]) == 1 || ( abs(crys[k]-GeCrys[j]) == 5  && abs(crys[k]+GeCrys[j]) == 5 ) || crys[k] == 6 || GeCrys[j] == 6){
                  hGe_dTaddback->Fill(tmp_dt);
                  adbk_E += GeE_Cal[j];
                  GeE_Cal[j]=-100;
                 // T[v] = T[v-1]/2+Ge_Talign[j]/2 ;
                  adbk_T = adbk_T/(v+1)*v+Ge_Talign[j]/(v+1) ;
                  Ge_Talign[j]=-100;
                  crys[v] = GeCrys[j];
                  v++;
                    }
                ...
                // v--;
               // Fill the addback branch         
                hGe_Chan_E_Addback[GeDet[i]][GeCrys[i]]->Fill(adbk_E); 
                pOutput->pGe_EAddback[GeDet[i]][GeCrys[i]] = adbk_E;
                pOutput->pGe_TAddback[GeDet[i]][GeCrys[i]] = adbk_T;
                hGe_AddbackSum->Fill(adbk_E);
                hGe_AddbackSum_halfkev->Fill(adbk_E);
            }
           }

//       for (int i = 0; i < Germanium_MAX_DETS-1; i++)
//       {
//           double E[Germanium_CRYSTALS] = { 0 };
//           ULong64_t T[Germanium_CRYSTALS] = { 0 };
...
//           for (int j = 0; j < Germanium_CRYSTALS; j++)
//               {
//             if (pOutput->pGe_E[i][j] < energy_thres_addback || pOutput->pGe_T[i][j] == 0) continue;
//             bool added = false; 
//       
//             // Try to addback to an existing hit
//             for (int k = 0; k < v; k++){
//               tmp_dt=T[k] - pOutput->pGe_T[i][j];
//              if (abs(tmp_dt) < dT_addback ){
//                for(int l = 0; l< n_o_h; l++){
//                  if( abs(crys[l]-j) == 1 || ( abs(crys[l]-j)==5 && crys[l]+j==5 ) || j == 6 || crys[l] == 6){
//                    hGe_dTaddback->Fill(tmp_dt);
//            //    cout<<tmp_dt<<endl;
//             //    if(tmp_dt<0) cin.get();
...
//           {
//               
//               hGe_Chan_E_Addback[i][crys[j]]->Fill(E[j]); 
//               pOutput->pGe_EAddback[i][crys[j]] = E[j];
//               pOutput->pGe_TAddback[i][crys[j]] = T[j];
//
...
//       }
               ...
               // gamma - gamma matrix after addback
       for (int i = 0; i < Germanium_MAX_DETS-1; i++){
        for (int j = 0; j < Germanium_CRYSTALS; j++){
         for (int ii = 0; ii < Germanium_MAX_DETS-1; ii++){
          for (int jj = 0; jj < Germanium_CRYSTALS; jj++){
          if(i==ii&&j==jj) continue; 
          tmp_dt=pOutput->pGe_TAddback[i][j]-pOutput->pGe_TAddback[ii][jj];
           if(pOutput->pGe_TAddback[i][j]>0 && pOutput->pGe_TAddback[ii][jj]>0 && pOutput->pGe_EAddback[i][j]>0 && pOutput->pGe_EAddback[ii][jj]>0 && abs(tmp_dt)<200){
            hGe_EE_Mat_2->Fill(pOutput->pGe_EAddback[i][j],pOutput->pGe_EAddback[ii][jj]) ;
           }
       } 
       }
        } 
