...
//        }
            ...
            fOutput->fbeta_vftx_2141[i] = RAW->get_FRS_vftx_beta_2141(i); 
            fOutput->fgamma_vftx_2141[i] = RAW->get_FRS_vftx_gamma_2141(i);
            fOutput->faoq_vftx_2141[i] = RAW->get_FRS_vftx_aoq_2141(i);
            fOutput->faoq_corr_vftx_2141[i] = RAW->get_FRS_vftx_aoq_2141_corr(i);
            fOutput->fz1_vftx_2141[i] = RAW->get_FRS_vftx_2141_z(i);
            fOutput->fz2_vftx_2141[i] = RAW->get_FRS_vftx_2141_z2(i);
            fOutput->fvcor_vftx_2141[i] = RAW->get_FRS_vftx_2141_v_cor(i);
            fOutput->fbeta_vftx_2241[i] = RAW->get_FRS_vftx_beta_2241(i);
            fOutput->fgamma_vftx_2241[i] = RAW->get_FRS_vftx_gamma_2241(i);
            fOutput->faoq_vftx_2241[i] = RAW->get_FRS_vftx_aoq_2241(i);
          
           // if(fOutput->faoq_vftx_2241[i]>0)cout<<"UNPACK EVENT "<< event_number << " fOutput->faoq_vftx_2241[i] " <<fOutput->faoq_vftx_2241[i] << endl;
            fOutput->faoq_corr_vftx_2241[i] = RAW->get_FRS_vftx_aoq_2241_corr(i);
            fOutput->fz1_vftx_2241[i] = RAW->get_FRS_vftx_2241_z(i);
            fOutput->fz2_vftx_2241[i] = RAW->get_FRS_vftx_2241_z2(i);
            fOutput->fvcor_vftx_2241[i] = RAW->get_FRS_vftx_2241_v_cor(i);
                }
             }
            ///SCI TOF
...
//         }
 ///Using TAC
	...
	if(RAW->get_FRS_id_mhtdc_aoq_corr()>0)   fOutput->fFRS_AoQ_corr_mhtdc = RAW->get_FRS_id_mhtdc_aoq_corr();
	if(RAW->get_FRS_id_mhtdc_z1()>0)      fOutput->fFRS_z_mhtdc = RAW->get_FRS_id_mhtdc_z1();
	if(RAW->get_FRS_id_mhtdc_z2()>0)  fOutput->fFRS_z2_mhtdc = RAW->get_FRS_id_mhtdc_z2();
	...
	if(RAW->get_FRS_id_mhtdc_beta()>0.0 &&  RAW->get_FRS_id_mhtdc_beta()<1.0)  fOutput->fFRS_beta_mhtdc = RAW->get_FRS_id_mhtdc_beta();
	
	if(RAW->get_FRS_id_mhtdc_tof4121()>0 )  fOutput->fFRS_tof4121_mhtdc = RAW->get_FRS_id_mhtdc_tof4121();
...
//         fOutput->fFRS_ts2 = RAW->get_FRS_ts2();
       ...
       // cout<<"Event AIDA " << event_number << " evt.Energy " << evt.Energy <<" evt.HighEnergy " << evt.HighEnergy <<endl;}
        ...
        int Phys_Channel_Lead_Fast_Fat[4][256];
        int Phys_Channel_Trail_Fast_Fat[4][256];
        int Phys_Channel_Lead_Slow_Fat[4][256];
        int Phys_Channel_Trail_Slow_Fat[4][256];
...
//cout<<"fOutput->fbPlas_Lead_PMT[bPlasdetnum][chan][N1] " <<fOutput->fbPlas_Lead_PMT[bPlasdetnum][chan][N1] << " chan " << chan << " N1 " << N1 << endl;
        ...
        int Fat_QDC_ID;
        int Fat_TDC_ID_sing;
        int Fat_TDC_ID[FAT_MAX_VME_CHANNELS];
        ...
        int sc41L_count = 0;
        int sc41R_count = 0;
        int sc41L_dig_count = 0;
        int sc41R_dig_count = 0;
	  ...
	  qdc_multi_hit_exclude[i] = 0;
         ...
         if(RAW->get_FAT_TDC_id(i) > -1 ){
	      ...
	      if(RAW->get_FAT_TDC_id(i)<FAT_MAX_VME_CHANNELS){
             ...
             if(RAW->get_FAT_TDC_id(i) == SC41L_FatVME && RAW->get_FAT_TDC_timestamp(i) != 0. ){
                fOutput->fSC41L[sc41L_count] =  RAW->get_FAT_TDC_timestamp(i);
                sc41L_count++;
               
            }// end if statement to check for SC41L hits

             if(RAW->get_FAT_TDC_id(i) == SC41R_FatVME && RAW->get_FAT_TDC_timestamp(i) != 0. ){
                fOutput->fSC41R[sc41R_count] =  RAW->get_FAT_TDC_timestamp(i);
                sc41R_count++;
               
            }// end if statement to check for SC41L hits

              if(RAW->get_FAT_TDC_id(i) == SC41L_FatVME_Digi && RAW->get_FAT_TDC_timestamp(i) != 0. ){
                fOutput->fSC41L_dig[sc41L_dig_count] =  RAW->get_FAT_TDC_timestamp(i);
                sc41L_dig_count++;
               
            }// end if statement to check for SC41L hits
            
             // if(RAW->get_FAT_TDC_id(i) == SC41R_FatVME && RAW->get_FAT_TDC_timestamp(i) != 0. ){
            if(RAW->get_FAT_TDC_id(i) == SC41R_FatVME_Digi && RAW->get_FAT_TDC_timestamp(i) !=0.){
                fOutput->fSC41R_dig[sc41R_dig_count] =  RAW->get_FAT_TDC_timestamp(i);
                sc41R_dig_count++;
        ...
        for (int i=0; i< RAW->get_FAT_QDCs_fired(); i++){
            ...
            for(int j = 0; j < RAW->get_FAT_QDCs_fired(); j++){
                if(RAW->get_FAT_QDC_id(i) == RAW->get_FAT_QDC_id(j) && i!=j && RAW->get_FAT_QDC_id(i)<FAT_MAX_VME_CHANNELS){
...
//cout << "event no: " << event_number << endl;
          ...
          fOutput->fFat_SC41L_mult =  sc41L_count;
          fOutput->fFat_SC41R_mult =  sc41R_count;
          fOutput->fFat_SC41L_dig_mult =  sc41L_dig_count;
          fOutput->fFat_SC41R_dig_mult =  sc41R_dig_count;
...
void EventUnpackProc::FILL_HISTOGRAMS(int PrcID_Conv, int PrcID, int SubType){
 ...
 // switch(PrcID_Conv){
  ...
  if(PrcID_Conv==3 && Used_Systems[3]) Fill_FATIMA_Histos();
...
//         hSCI_Tof3 = MakeH1I(fname,name,1000,0,62000,"TAC SC42-SC21 [ps] (pos.corr.)",2,3);
    ...
    hID_Z_Z2_mhtdc = MakeTH2('D',"FRS/MHTDC/ID/ID_Z1_Z2_mhtdc","Z1 vs. Z2", 1000,FRS_MIN_Z,FRS_MAX_Z, 1000,FRS_MIN_Z,FRS_MAX_Z,"Z1", "Z2");
     ...
     h_VFTX_beta2141 = MakeTH1('D',"FRS/VFTX/ID/ID_2141_Beta_vftx", "VFTX Beta 21->41",2000,0,2000, "Beta*1000");
     h_VFTX_beta2241 = MakeTH1('D',"FRS/VFTX/ID/ID_2241_Beta_vftx", "VFTX Beta 22->41",2000,0,2000, "Beta*1000");
     
     h_VFTX_x2AoQ2141= MakeTH2('D',"FRS/VFTX/ID/ID_x2_AoQ_S21-S41_vftx", "ID_x2_AoQ_S21-S41 vftx",1500,FRS_MIN_AoQ,FRS_MAX_AoQ, 200,-100.,100., "A/Q s21-s41", "X at S2 [mm]");
     
     h_VFTX_x2AoQ2241= MakeTH2('D',"FRS/VFTX/ID/ID_x2_AoQ_S22-S41_vftx", "ID_x2_AoQ_S22-S41 vftx",1500,FRS_MIN_AoQ,FRS_MAX_AoQ, 200,-100.,100., "A/Q s22-s41", "X at S2 [mm]");
     
    h_VFTX_AoQ2141 = MakeTH1('D',"FRS/VFTX/ID/ID_2141_AoQ_vftx", "VFTX A/Q 21->41",1500,FRS_MIN_AoQ,FRS_MAX_AoQ, "A/Q s2-s4");
    h_VFTX_AoQ2141corr = MakeTH1('D',"FRS/VFTX/ID/ID_2141_AoQcorr_vftx", "VFTX A/Q Corr 21->41",1500,FRS_MIN_AoQ,FRS_MAX_AoQ, "A/Q s2-s4");
   
    h_VFTX_AoQ2241 = MakeTH1('D',"FRS/VFTX/ID/ID_2241_AoQ_vftx", " VFTX A/Q 22->41",1500,FRS_MIN_AoQ,FRS_MAX_AoQ, "A/Q s2-s4");
    h_VFTX_AoQ2241corr = MakeTH1('D',"FRS/VFTX/ID/ID_Z1_2241_AoQ_corr_vftx","A/QCorr 22->41",1500,FRS_MIN_AoQ,FRS_MAX_AoQ, "A/Q s2-s4");
    
     h_VFTX_Z1_AoQ2141 = MakeTH2('D',"FRS/VFTX/ID/ID_Z1_AoQ_S21-S41", "ID_Z1_AoQ_S21-S41",1500,FRS_MIN_AoQ,FRS_MAX_AoQ, 1000,FRS_MIN_Z,FRS_MAX_Z, "A/Q s2-s4", "Z s2-s4");
    
     h_VFTX_Z1_AoQ2141corr = MakeTH2('D',"FRS/VFTX/ID/ID_Z1_AoQ_S21-S41corr", "ID_Z1_AoQ_S21-S41corr",1500,FRS_MIN_AoQ,FRS_MAX_AoQ, 1000,FRS_MIN_Z,FRS_MAX_Z, "A/Q s2-s4", "Z s2-s4");
     
     h_VFTX_Z1_AoQ2241 = MakeTH2('D',"FRS/VFTX/ID/ID_Z1_AoQ_S22-S41", "ID_Z1_AoQ_S22-S41",1500,FRS_MIN_AoQ,FRS_MAX_AoQ, 1000,FRS_MIN_Z,FRS_MAX_Z, "A/Q s2-s4", "Z s2-s4");
    
     h_VFTX_Z1_AoQ2241corr = MakeTH2('D',"FRS/VFTX/ID/ID_Z1_AoQ_S22-S41corr", "ID_Z1_AoQ_S22-S41corr",1500,FRS_MIN_AoQ,FRS_MAX_AoQ, 1000,FRS_MIN_Z,FRS_MAX_Z, "A/Q s2-s4", "Z s2-s4");
     
     h_VFTX_Z_2141 = MakeTH1('D',"FRS/VFTX/ID/ID_Z_2141_vftx","ID_Z1",1000,FRS_MIN_Z,FRS_MAX_Z,"Z1 s2-s4");
     h_VFTX_Z2_2141 = MakeTH1('D',"FRS/VFTX/ID/ID_Z2_2141_vftx","ID_Z2",1000,FRS_MIN_Z,FRS_MAX_Z,"Z2 s2-s4");
   
      h_VFTX_Z_2241 = MakeTH1('D',"FRS/VFTX/ID/ID_Z_2241_vftx","ID_Z1",1000,FRS_MIN_Z,FRS_MAX_Z,"Z1 s2-s4");
      h_VFTX_Z2_2241 = MakeTH1('D',"FRS/VFTX/ID/ID_Z2_2241_vftx","ID_Z2",1000,FRS_MIN_Z,FRS_MAX_Z,"Z2 s2-s4");
      
      h_VFTX_Z1Z2_2141 = MakeTH2('D',"FRS/VFTX/ID/ID_Z1Z2_S21-S41corr", "ID_Z1Z2_S21-S41corr",1000,FRS_MIN_Z,FRS_MAX_Z, 1000,FRS_MIN_Z,FRS_MAX_Z, "A/Q s2-s4", "Z s2-s4");
      
      h_VFTX_Z1Z2_2241 = MakeTH2('D',"FRS/VFTX/ID/ID_Z1Z2_S22-S41corr", "ID_Z1Z2_S22-S41corr",1000,FRS_MIN_Z,FRS_MAX_Z, 1000,FRS_MIN_Z,FRS_MAX_Z, "A/Q s2-s4", "Z s2-s4");
  ...
  //   hID_Z = MakeH1I("ID",Form("ID_Z, gain=%f",music->e1_gain[0]),1000,10,93,"Z s2-s4",2,6);
   ...
   // hID_Z2_vftx = MakeTH1('D',"FRS/MHTDC/ID/ID_Z2_mhtdc","ID_Z2",1000,FRS_MIN_Z,FRS_MAX_Z,"Z2 s2-s4");
...
//          hID_dEdeg_Z1_Z1AoQgate[i] = MakeTH2('D',"FRS/ID_Gated/ID_dEdeg_Z1/dEdeg_Z1_Z1AoQgated",  name,  1000,FRS_MIN_Z,FRS_MAX_Z, 1000, 10.,50., "Z1 from MUSIC41", "dE(S2deg) [a.u.]");
...
//     htpc_Y4 = MakeTH1('D',"FRS/TPC/S4_TPCY","tpc_y S41",800,-100,100);
    ...
    hID_beta_mhtdc= MakeTH1('D',"FRS/MHTDC/ID/ID_beta_mhtdc","ID_beta",2000,0,2000,"beta mhtdc*1000");
  ...
  //  hMUSIC1_MUSIC2 = MakeTH2('D',"FRS/MUSIC/MUSIC1_MUSIC2","dE1_dE2", 1024,0,4096,1024,0,4096);  
    ...
    for(int i=0;i<32;i++)
     {
       hvftx_TRaw[i] = MakeTH1('D', Form("FRS/VFTX/TRaw/TRaw%2d",i), Form("T Raw %2d",i), 2000,1E6,1E8);
     }
   hvftx_Hit_Pattern= MakeTH1('D',"FRS/VFTX/HitPattern","Hit Pattern",16,0.,16);
   
   hvftx_Sci21PosRaw = MakeTH1('D',"FRS/VFTX/PosRaw21","Sci21_PosRaw",1000,-20000.,20000,"PosRaw = Left - Right [10ps/bin]");
   hvftx_Sci22PosRaw = MakeTH1('D',"FRS/VFTX/PosRaw22","Sci22_PosRaw",1000,-20000.,20000,"PosRaw = Left - Right [10ps/bin]");
  
   //hvftx_Sci21PosRaw_TRAW = MakeTH1('D',"FRS/VFTX/PosRaw21_TRAW","Sci21_PosRaw_TRAW",1000,-20000.,20000,"PosRaw = Left - Right [10ps/bin]");
  
   hvftx_Sci41PosRaw = (TH1D*)MakeTH1('D',"FRS/VFTX/PosRaw41","Sci41_PosRaw",1000,-20000.,20000,"PosRaw = Left - Right [10ps/bin]"); 
   hvftx_Sci42PosRaw = (TH1D*)MakeTH1('D',"FRS/VFTX/PosRaw42","Sci42_PosRaw",1000,-20000.,20000,"PosRaw = Left - Right [10ps/bin]"); 
...
//     h1_VFTX_TOF_41_22_calib = (TH1D*)MakeTH1('D',"FRS/VFTX/TOF/Sci2241_calib","TOF_Sci22_Sci41 calib",8000,100000.,400000.,"TOF = Sci41 - Sci22 [1ps/bin]");
//     h1_VFTX_TOF_42_21_calib = (TH1D*)MakeTH1('D',"FRS/VFTX/TOF/Sci2142_calib","TOF_Sci22_Sci41 calib",8000,100000.,400000.,"TOF = Sci42 - Sci21 [1ps/bin]");
...
//     ToF_vftx_2142_calib[i] =0;
   ...
   if(PrcID==20){
    AoQ_mhtdc = RAW->get_FRS_id_mhtdc_aoq();
    AoQ_corr_mhtdc = RAW->get_FRS_id_mhtdc_aoq_corr();
    ID_z_mhtdc = RAW->get_FRS_id_mhtdc_z1();
    ID_z2_mhtdc = RAW->get_FRS_id_mhtdc_z2();
    beta_mhtdc = RAW->get_FRS_id_mhtdc_beta();
    ID_tof4121_mhtdc = RAW->get_FRS_id_mhtdc_tof4121();
...
//     ToF_vftx_2142_calib[i] = RAW->get_FRS_ToF_vftx_2142_calib(i);
    ...
    FRS_vftx_gamma_2141[i] = RAW->get_FRS_vftx_gamma_2141(i);
    FRS_vftx_aoq_2141[i] = RAW->get_FRS_vftx_aoq_2141(i);
    FRS_vftx_aoq_2141_corr[i] = RAW->get_FRS_vftx_aoq_2141_corr(i);
    FRS_vftx_2141_z[i] = RAW->get_FRS_vftx_2141_z(i);
    FRS_vftx_2141_z2[i] = RAW->get_FRS_vftx_2141_z2(i);
    FRS_vftx_2141_v_cor[i] = RAW->get_FRS_vftx_2141_v_cor(i);
    FRS_vftx_beta_2241[i] = RAW->get_FRS_vftx_beta_2241(i);
    FRS_vftx_gamma_2241[i] = RAW->get_FRS_vftx_gamma_2241(i);
    FRS_vftx_aoq_2241[i] = RAW->get_FRS_vftx_aoq_2241(i);
//   if(FRS_vftx_aoq_2241[i]>0) cout<<"FRS_vftx_aoq_2241[i] " <<FRS_vftx_aoq_2241[i] << endl;
    FRS_vftx_aoq_2241_corr[i] = RAW->get_FRS_vftx_aoq_2241_corr(i);
    FRS_vftx_2241_z[i] = RAW->get_FRS_vftx_2241_z(i);
    FRS_vftx_2241_z2[i] = RAW->get_FRS_vftx_2241_z2(i);
    FRS_vftx_2241_v_cor[i] = RAW->get_FRS_vftx_2241_v_cor(i);
    }
...
//      hID_Z_Z2->Fill(ID_z,ID_z2);
    ...
    // hID_Z_Z3->Fill(ID_z,ID_z3);
     ...
     ///MHTDC
     if(AoQ_mhtdc!=0){
     if(ID_tof4121_mhtdc!=0){
         hMultiHitTDC_TOF_41_21->Fill(ID_tof4121_mhtdc);
    //  if(ID_tof4121_mhtdc>147.3 && ID_tof4121_mhtdc<147.73)   
     ...
     if(AoQ_mhtdc!=0){hID_AoQ_mhtdc->Fill(AoQ_mhtdc);
         // cout<<"AoQ_mhtdc " << AoQ_mhtdc<<endl;
     }
     if(AoQ_corr_mhtdc!=0)hID_AoQ_corr_mhtdc->Fill(AoQ_corr_mhtdc);
     if(ID_z_mhtdc!=0)hID_Z_mhtdc->Fill(ID_z_mhtdc);
     if(ID_z2_mhtdc!=0)hID_Z2_mhtdc->Fill(ID_z2_mhtdc);
     if(beta_mhtdc!=0)hID_beta_mhtdc->Fill(beta_mhtdc);
     if(ID_z_mhtdc!=0 &&AoQ_mhtdc!=0 )hID_Z_AoQ_mhtdc->Fill(AoQ_mhtdc,ID_z_mhtdc);
    if(ID_z_mhtdc!=0 &&AoQ_corr_mhtdc!=0)hID_Z_AoQ_corr_mhtdc->Fill(AoQ_corr_mhtdc,ID_z_mhtdc);
         
     
     if(ID_z_mhtdc!=0&& ID_z2_mhtdc!=0)hID_Z_Z2_mhtdc->Fill(ID_z_mhtdc,ID_z2_mhtdc);
    }
}
...
//  cout<<"event " << event_number << " TRaw_vftx_41l " << TRaw_vftx_41l << " TRaw_vftx_41r " << TRaw_vftx_41r<<" TRaw_vftx_41l-TRaw_vftx_41r " <<TRaw_vftx_41l-TRaw_vftx_41r << endl; 
   ...
   if(ToF_vftx_2141[i]!=0) h1_VFTX_TOF_41_21->Fill(ToF_vftx_2141[i]);
    ...
    if(ToF_vftx_2141_calib[i]!=0) h1_VFTX_TOF_41_21->Fill(ToF_vftx_2141_calib[i]);
    if(ToF_vftx_2241_calib[i]!=0) h1_VFTX_TOF_41_22->Fill(ToF_vftx_2241_calib[i]);
...
//         }
    ...
    if(FRS_vftx_beta_2141[i]!=0) h_VFTX_beta2141->Fill(FRS_vftx_beta_2141[i]*1000.);
    if(FRS_vftx_aoq_2141[i]!=0)h_VFTX_AoQ2141->Fill(FRS_vftx_aoq_2141[i]);
	if(FRS_vftx_aoq_2141_corr[i]!=0)h_VFTX_AoQ2141corr ->Fill(FRS_vftx_aoq_2141_corr[i]);
    if(FRS_vftx_aoq_2141[i]!=0)h_VFTX_x2AoQ2141->Fill(FRS_vftx_aoq_2141[i], ID_x2);
    if(FRS_vftx_2141_z[i]!=0)h_VFTX_Z1_AoQ2141->Fill( FRS_vftx_aoq_2141[i],FRS_vftx_2141_z[i]);
    if(FRS_vftx_2141_z[i]!=0)h_VFTX_Z1_AoQ2141corr->Fill( FRS_vftx_aoq_2141_corr[i],FRS_vftx_2141_z[i]);
    if(FRS_vftx_beta_2241[i]!=0)h_VFTX_beta2241->Fill(FRS_vftx_beta_2241[i]*1000.);
	if(FRS_vftx_aoq_2241[i]!=0)h_VFTX_AoQ2241->Fill(FRS_vftx_aoq_2241[i]);
	if(FRS_vftx_aoq_2241_corr[i]!=0)h_VFTX_AoQ2241corr->Fill(FRS_vftx_aoq_2241_corr[i]);
    if(FRS_vftx_aoq_2241[i]!=0)h_VFTX_x2AoQ2241->Fill(FRS_vftx_aoq_2241[i], ID_x2);
    if(FRS_vftx_2241_z[i]!=0)h_VFTX_Z1_AoQ2241->Fill( FRS_vftx_aoq_2241[i],FRS_vftx_2241_z[i]);
    if(FRS_vftx_2241_z[i]!=0)h_VFTX_Z1_AoQ2241corr->Fill( FRS_vftx_aoq_2241_corr[i],FRS_vftx_2241_z[i]);
    if(FRS_vftx_2141_z[i]!=0)h_VFTX_Z_2141->Fill(FRS_vftx_2141_z[i]);
    if(FRS_vftx_2241_z[i]!=0)h_VFTX_Z_2241->Fill(FRS_vftx_2241_z[i]);
    if(FRS_vftx_2141_z2[i]!=0)h_VFTX_Z2_2141->Fill(FRS_vftx_2141_z2[i]);
    if(FRS_vftx_2241_z2[i]!=0)h_VFTX_Z2_2241->Fill(FRS_vftx_2241_z2[i]);
    if(FRS_vftx_2141_z[i]!=0)h_VFTX_Z1Z2_2141->Fill(FRS_vftx_2141_z[i],FRS_vftx_2141_z2[i]);
    if(FRS_vftx_2241_z[i]!=0)h_VFTX_Z1Z2_2241->Fill(FRS_vftx_2241_z[i],FRS_vftx_2241_z2[i]);
...
void EventUnpackProc::Fill_FATIMA_Histos(){
  ...
  for (int i=0; i<RAW->get_FAT_TDCs_fired(); i++){ /** Loops over only channels in the TDC 1-4 **/
   ...
   // cout<<detTDC << endl;
   // zgx
     if(detTDC<=FAT_MAX_VME_CHANNELS&&RAW->get_FAT_TDC_timestamp(i) != 0.){
        FAT_T[detTDC] = (RAW->get_FAT_TDC_timestamp(i));
        hFAT_Traw_VME[detTDC]->Fill(FAT_T[detTDC]*25); //in ps
//      if(detTDC==38){cout<<"Fired "<<i<<" FAT_T[detTDC] " <<FAT_T[detTDC] <<" detTDC " << detTDC<< endl;
    //cin.get();
//      }
//      if(detTDC==39){cout<<"Fired "<<i<<" FAT_T[detTDC] " <<FAT_T[detTDC] <<" detTDC " << detTDC<< endl; 
    //cin.get();
//      }
     //zgx
                }
...
//-----------------------------------------------------------------------------------------//
   ...
   if(MHTDC_TAC_VFTX==1)  cout<<"\tFRS: MHTDC used for analysis  " << endl;
   if(MHTDC_TAC_VFTX==0)  cout<<"\tFRS: NIM TAC used for analysis  " << endl;
   if(MHTDC_TAC_VFTX==2)  cout<<"\tFRS: VFTX used for analysis  " << endl;
   cout<<"\n////////////////////////////////////////////////////////"<<endl;
